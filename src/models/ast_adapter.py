...
class ASTAdapter(nn.Module):
    ...
    def __init__(self, label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, 
                 imagenet_pretrain=True, audioset_pretrain=True, model_size='base384', verbose=True, adapter_shared=True,
                  RR=128, adapter_mode='parallel', adapter_location='ffn'):

        super(ASTAdapter, self).__init__()
        ...
        # now load a model that is pretrained on both ImageNet and AudioSet
            ...
            audio_model = ASTAdapter(label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, imagenet_pretrain=False, audioset_pretrain=False, model_size='base384', verbose=False)
    ...
    def freeze_base_model(self):
        ...
        if hasattr(self, 'adapters_mhsa') and self.adapters_mhsa is not None:
            for adapter in self.adapters_mhsa:
                for param in adapter.parameters():
                    param.requires_grad = True
        
        if hasattr(self, 'adapters_ffn') and self.adapters_ffn is not None:
            for adapter in self.adapters_ffn:
                for param in adapter.parameters():
                    param.requires_grad = True
