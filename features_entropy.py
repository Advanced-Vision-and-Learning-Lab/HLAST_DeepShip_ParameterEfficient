...
vs_logmel_data = np.load('tb_logs/vs_logmel_features.npz')
...
vs_logmel_features = vs_logmel_data['vs_logmel_features']
...
vs_logmel_features = vs_logmel_features.squeeze(1) 
vs_logmel_features = np.transpose(vs_logmel_features, (0, 2, 1))
...
vs_logmel_sample = vs_logmel_features[0] 
...
# # Plot Log Mel Filter Bank
# plt.figure(figsize=(8, 6))
# plt.imshow(vs_logmel_sample, aspect='auto', cmap='gray', origin='lower')
# plt.colorbar(label='Magnitude')
# # Set title and labels with increased font size
# plt.title('vs Log Mel Filter Bank Feature', fontsize=16)
# plt.xlabel('Time Frames', fontsize=14)
# plt.ylabel('Mel Frequency Bins', fontsize=14)
# # Adjust tick parameters for x and y axes
# plt.xticks(fontsize=12)
# plt.yticks(fontsize=12)
# plt.savefig('features/vs_logmel_feature_sample.png', dpi=300)
# plt.close()
...
# vs_logmel_entropy = compute_entropy(vs_logmel_sample)
# # Print the results
...
# print(f'vs Log Mel Filter Bank Sample Entropy: {vs_logmel_entropy:.4f}')
...
# vs_logmel_hist, vs_logmel_bin_edges = compute_histogram(vs_logmel_sample)
...
# # Plot Probability Distribution of s Log Mel Filter Bank Sample
# plt.figure(figsize=(8, 6))
# plt.plot(vs_logmel_bin_edges[:-1], vs_logmel_hist, label='vs Log Mel Filter Bank', color='orange')
# plt.fill_between(vs_logmel_bin_edges[:-1], vs_logmel_hist, alpha=0.3, color='orange')
# plt.title('Probability Distribution of vs Log Mel Filter Bank Feature Sample', fontsize=16)
# plt.xlabel('Feature Values', fontsize=14)
# plt.ylabel('Probability Density', fontsize=14)
# plt.grid(True)
# plt.legend(fontsize=14)
# plt.savefig('features/vs_logmel_prob_distribution.png', dpi=300)
# plt.close()
...
# vs_logmel_entropy_multiple, vs_logmel_m_std = compute_entropy_multiple(vs_logmel_features)
...
# print(f'Average vs Log Mel Filter Bank Entropy: {vs_logmel_entropy_multiple:.4f}, std: {vs_logmel_m_std}')


from scipy.stats import skew, kurtosis

def compute_entropy_skew_kurtosis(data_samples):
    entropies = []
    skews = []
    kurtoses = []
    ...
    for sample in data_samples:
        ...
        # sample_skew = skew(data_flat)
        # sample_kurtosis = kurtosis(data_flat)

        sample_skew = skew(hist)
        sample_kurtosis = kurtosis(hist)
        
        entropies.append(ent)
        skews.append(sample_skew)
        kurtoses.append(sample_kurtosis)
...
# Compute entropy, skewness, and kurtosis for multiple samples
...
print("Skewness Mean:", logmel_stats['skew_mean'])
print("Skewness Std:", logmel_stats['skew_std'])
print("Kurtosis Mean:", logmel_stats['kurtosis_mean'])
print("Kurtosis Std:", logmel_stats['kurtosis_std'])

print('\n s logmel:')
print("Entropy Mean:", s_logmel_stats['entropy_mean'])
print("Entropy Std:", s_logmel_stats['entropy_std'])
print("Skewness Mean:", s_logmel_stats['skew_mean'])
print("Skewness Std:", s_logmel_stats['skew_std'])
print("Kurtosis Mean:", s_logmel_stats['kurtosis_mean'])
print("Kurtosis Std:", s_logmel_stats['kurtosis_std'])

print('\n vs logmel:')
print("Entropy Mean:", vs_logmel_stats['entropy_mean'])
print("Entropy Std:", vs_logmel_stats['entropy_std'])
print("Skewness Mean:", vs_logmel_stats['skew_mean'])
print("Skewness Std:", vs_logmel_stats['skew_std'])
print("Kurtosis Mean:", vs_logmel_stats['kurtosis_mean'])
print("Kurtosis Std:", vs_logmel_stats['kurtosis_std'])
...
# # Compute mean and std over the entire batch
# mean, std = compute_batch_stats(vs_logmel_features)
# # Normalize the batch using the computed mean and std
# normalized_vs_logmel_features = normalize_batch(vs_logmel_features, mean, std)
# print(f"vs log mel Mean: {mean:.4f}, Std: {std:.4f}")
